...
export const appwriteConfig = {
  ...
  contirbutorId: process.env.EXPO_PUBLIC_CONTRIBUTE,
};
let platformID;
if (Platform.OS === "ios") {
  // Platform is iOS
...
//get the wallpaper and contributor
export async function getwallpaper() {
  try {
    const posts = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.contirbutorId,
...
export async function addComment(postID, username, userID, comment) {
  // alert(comment);
  try {
    // First, create a comment document in commentsCollectionId
